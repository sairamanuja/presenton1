    ...
    async def _generate_image_google(
        ...
        try:
            ...
            if not response or not response.candidates:
                raise HTTPException(
                    status_code=500, detail=f"No candidates in response from {model}"
                )
            ...
            raise HTTPException(
                status_code=500, detail=f"Error generating image with {model}: {str(e)}"
            )

    async def generate_image_gemini_flash(
        self, prompt: str, output_directory: str
    ) -> str:
        """Gemini Flash doesn't support image generation. Fallback to Pexels."""
        print(f"Gemini Flash doesn't support images. Using Pexels for: {prompt}")
        return await self.get_image_from_pexels(prompt)

    async def generate_image_nanobanana_pro(
        self, prompt: str, output_directory: str
    ) -> str:
        """NanoBanana Pro image models don't exist. Fallback to Pexels."""
        print(f"NanoBanana Pro image models unavailable. Using Pexels for: {prompt}")
        return await self.get_image_from_pexels(prompt)

    async def get_image_from_pexels(self, prompt: str) -> str:
        try:
            pexels_key = get_pexels_api_key_env()
            if not pexels_key:
            ...
            async with aiohttp.ClientSession(trust_env=True) as session:
                ...
                if response.status != 200:
                    raise HTTPException(
                        status_code=500, 
                        detail=f"Pexels API error: {response.status}"
                    )
                ...
                if not data or "photos" not in data or not data["photos"]:
                    raise HTTPException(
                        status_code=500,
                        detail=f"No images found on Pexels for query: {prompt}"
                    )
                ...
                if not image_url:
                    raise HTTPException(
                        status_code=500,
                        detail="Invalid image URL from Pexels"
                    )
            ...
            raise HTTPException(
                status_code=500, 
                detail=f"Error fetching image from Pexels: {str(e)}"
            )

    async def get_image_from_pixabay(self, prompt: str) -> str:
        try:
            pixabay_key = get_pixabay_api_key_env()
            if not pixabay_key:
                raise ValueError("PIXABAY_API_KEY is not set")

            async with aiohttp.ClientSession(trust_env=True) as session:
                response = await session.get(
                    f"https://pixabay.com/api/?key={pixabay_key}&q={prompt}&image_type=photo&per_page=3",
                    timeout=aiohttp.ClientTimeout(total=10),
                )
                ...
                if response.status != 200:
                    raise HTTPException(
                        status_code=500, 
                        detail=f"Pixabay API error: {response.status}"
                    )
                ...
                if not data or "hits" not in data or not data["hits"]:
                    raise HTTPException(
                        status_code=500,
                        detail=f"No images found on Pixabay for query: {prompt}"
                    )
                ...
                if not image_url:
                    raise HTTPException(
                        status_code=500,
                        detail="Invalid image URL from Pixabay"
                    )
            ...
            raise HTTPException(
                status_code=500, 
                detail=f"Error fetching image from Pixabay: {str(e)}"
            )
