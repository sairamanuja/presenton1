...
class ImageGenerationService:
    ...
    async def _generate_image_google(
        ...
        for part in response.candidates[0].content.parts:
            if part.inline_data is not None:
                image = part.as_image()
                # Save with .png extension since Gemini returns PNG format
                image_path = os.path.join(output_directory, f"{uuid.uuid4()}.png")
    ...
    async def _generate_image_google_images(
        self, prompt: str, output_directory: str, model: str
    ) -> str:
        """Generate image using Google image generation API."""
        client = genai.Client()
        response = await asyncio.to_thread(
        ...
        with open(image_path, "wb") as image_file:
            image_file.write(image.image_bytes or b"")
    ...
    async def generate_image_nanobanana_pro(
        ...
        # NanoBanana Pro should call the actual NanoBanana API, not Gemini
        raise NotImplementedError(
            "NanoBanana Pro image generation is not properly implemented. "
            "It currently tries to use Gemini models which don't support image generation. "
            "NanoBanana Pro should call https://api.nanobanana.com/v1/images/generate instead."
        )
